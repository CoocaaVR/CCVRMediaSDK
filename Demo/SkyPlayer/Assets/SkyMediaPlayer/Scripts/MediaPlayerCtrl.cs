...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    public GameObject m_TargetMaterial_2 = null;
    private Texture2D m_VideoTexture = null;
    private Texture2D m_VideoTextureDummy = null;
    public static MEDIAPLAYER_STATE m_CurrentState;
    private int m_iCurrentSeekPosition;
    public static float voltest;
    ...
    public static int mBufferingPercent = 0;
    ...
    public String pkgName = "com.coocaa.vr";
    ...
    //public GameObject m_objResize_2 = null;
    ...
    void Start()
    {
        //Debug.Log("player ctrl start");
        Call_SetUnityActivity();
        voltest = getCurrenatAudioVolume();
    ...
    void Update()
    {
        ...
        if( m_CurrentState != Call_GetStatus() )
        {
            ...
            if( m_CurrentState == MEDIAPLAYER_STATE.PREPARED )
            {
                mBufferingPercent = 0;
                if( OnReady != null )
                    OnReady();
            ...
            else if( m_CurrentState == MEDIAPLAYER_STATE.COMPLETED )
            {
                if( OnEnd != null )
                    OnEnd();
            ...
            if( m_CurrentState == MEDIAPLAYER_STATE.BUFFERING_PROCESS )
            {
                mBufferingPercent = Call_GetBufferingPercent();
                Debug.Log("mBufferingPercent : " + mBufferingPercent);
            }
                ...
                if(fRatio < 1.0f)
                {
                    if( fRatioScreen < 1.0f)
                    {
                        if( fRatio > fRatioScreen)
                        {
                            ...
                            m_objResize_1.transform.localScale *= fRatioResult;
                        }
                    }
...
*/
    ...
    void OnDestroy()
    {
        ...
        Call_Destroy();
        m_TargetMaterial.GetComponent<Renderer>().material.mainTexture = (Texture2D)Resources.Load("Dummy");
        m_TargetMaterial_2.GetComponent<Renderer>().material.mainTexture = (Texture2D)Resources.Load("Dummy");
    ...
    public void Play()
    {
        ...
        else if( m_CurrentState == MEDIAPLAYER_STATE.PREPARED || m_CurrentState == MEDIAPLAYER_STATE.STOPPED || m_CurrentState == MEDIAPLAYER_STATE.COMPLETED )
        {
            Call_Play(0);
        }
    }
    ...
    {
        Debug.Log("playr ctrl load str=" + strFileName);
        m_bCheckFBO = false;
        ...
        m_CurrentState = MEDIAPLAYER_STATE.IDLE;
    }
    ...
    public float getCurrenatAudioVolume()
    {
        Debug.Log("public getCurVol ");
        return Call_getCurrenatAudioVolume();
    }
	...
	private void Call_UpdateVideoTexture()
	{
		...
		if(m_TargetMaterial)
		{
			if(m_TargetMaterial.GetComponent<MeshRenderer>().material.mainTexture != m_VideoTexture)
			{
                ...
                m_TargetMaterial_2.GetComponent<MeshRenderer>().material.mainTexture = m_VideoTexture;
			}
	...
	private int Call_GetDuration()
	{
     ...
     private float Call_getCurrenatAudioVolume()
    {
        
        return GetJavaObject().Call<float>("getCurrentAudioVolume");
        Debug.Log("call  java  getCurVol");
	}
	 ...
	 private void Call_SetUnityActivity()
    {
        ...
        GetJavaObject().Call("SetUnityActivity", jo,pkgName);
    }
	...
	public int Call_GetBufferingPercent()
	{
		return GetJavaObject().Call<int>("GetBufferingPercent");
		Debug.Log("call  java  GetBufferingPercent");
	}
	...
	private MEDIAPLAYER_STATE Call_GetStatus()
	{
		...
		{
			return MEDIAPLAYER_STATE.COMPLETED;
		}
		...
		{
			return MEDIAPLAYER_STATE.PREPARED;
		}
    ...
    private float Call_getCurrenatAudioVolume()
    {
        Debug.Log("call getCurVol return -1");
        return -1;
    }
